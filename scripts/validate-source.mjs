...
const authRoot=read('src/auth/AuthContext.tsx'), authGate=read('src/auth/AuthGate.tsx'), login=read('src/routes/login.tsx'), changePassword=read('src/routes/alterar-senha.tsx'), reportService=read('src/services/reportService.ts');
assert(rootRoute.includes('<AuthProvider>')&&rootRoute.includes('<AuthGate>'),'V5.2: AuthProvider/AuthGate ausentes');
assert(login.includes('ancar-auth-v1/login')===false && login.includes('useAuth'),'V5.2: login deve usar o serviço de autenticação via contexto');
assert(changePassword.includes('minLength={6}')&&changePassword.includes('maxLength={72}'),'V5.2: política de senha 6-72 ausente no frontend');
assert(authGate.includes('/login')&&authGate.includes('/alterar-senha')&&authGate.includes('<AppLayout>'),'V5.2: gate de autenticação incompleto');
assert(svc.includes('Authorization: `Bearer ${token}`')&&svc.includes('signalAuthExpired'),'V5.2: APIs de dashboard sem Bearer/expiração');
assert(reports.includes('Relatório diário')&&reports.includes('Relatório semanal')&&reports.includes('Relatório mensal'),'V5.2: modelos PDF ausentes');
assert(reports.includes('Até os 6 meses fechados mais recentes existentes na base.'),'V5.2: histórico mensal de 6 meses ausente');
assert(reportService.includes('ancar-report-periods-v1')&&reportService.includes('ancar-report-monthly-pdf-v1'),'V5.2: serviço de relatórios PDF incompleto');

for(const route of ['index.tsx','shoppings.tsx','shoppings.$shoppingId.tsx','ranking.tsx','analises.tsx','alertas.tsx','esg.tsx','relatorios.tsx','configuracoes.tsx','login.tsx','alterar-senha.tsx']) assert(fs.existsSync(path.join(src,'routes',route)),`Rota ausente ${route}`);
