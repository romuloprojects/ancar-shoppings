...
m=re.search(r'const OVERVIEW_LAYOUT_V44_CSS = `([\s\S]*?)`;', route)
if not m:
    raise AssertionError('OVERVIEW_LAYOUT_V44_CSS não encontrado')
v44_css=m.group(1)
required=['@media (min-width: 1024px)','grid-template-rows: repeat(2, 146px) !important','height: 302px !important','height: 408px !important','grid-template-rows: 408px !important','overflow-y: auto !important']
missing=[x for x in required if x not in v44_css]
if missing: raise AssertionError('Regras V4.4 ausentes: '+', '.join(missing))
if 'min-height: 650px' in v44_css: raise AssertionError('V4.4 ainda depende de min-height')
...
print(f'VALIDAÇÃO PORTFÓLIO V4.4: PASS — {len(results)} viewports, incluindo alturas <650px')
