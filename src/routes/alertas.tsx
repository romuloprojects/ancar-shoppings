...
function AlertsPage(){
 ...
 <div className="grid grid-cols-2 gap-3 xl:grid-cols-4"><StatCard label="Alertas ativos" value={alerts.length} icon={Bell} accent="cyan"/><StatCard label="Críticos" value={critical} icon={ShieldAlert} accent={critical?"red":"green"}/><StatCard label="Atenção" value={attention} icon={AlertTriangle} accent={attention?"yellow":"green"}/><StatCard label="Sem alertas" value={alerts.length===0?"Sim":"Não"} icon={CheckCircle2} accent={alerts.length===0?"green":"blue"}/></div>
 <FilterBar><div className="relative min-w-[260px] flex-1"><Search className="pointer-events-none absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground"/><Input value={q} onChange={e=>setQ(e.target.value)} placeholder="Buscar alerta ou shopping" className="h-9 bg-background/55 pl-9"/></div><select value={severity} onChange={e=>setSeverity(e.target.value)} className="h-9 rounded-lg border border-border/60 bg-background/55 px-3 text-sm"><option value="todos">Todas as severidades</option><option value="critico">Crítico</option><option value="atencao">Atenção</option><option value="informativo">Informativo</option></select></FilterBar>
