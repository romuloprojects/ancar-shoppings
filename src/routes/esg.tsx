...
function ESGPage(){
 ...
 const [items,setItems]=useState<LiveShoppingSummary[]>([]); const [detail,setDetail]=useState<ShoppingApiResponse|null>(null); const [loading,setLoading]=useState(true);
 ...
 useEffect(()=>{if(!selectedShoppingCode)return;let alive=true;liveDashboardService.getShopping(selectedShoppingCode,historyPeriod).then(r=>{if(alive)setDetail(r)}).catch(()=>{if(alive)setDetail(null)});return()=>{alive=false}},[selectedShoppingCode,historyPeriod,tick]);
 ...
 <div className="energy-workspace-body min-h-0 flex-1">
   <SectionPanel title={`Energia real e estimativas · ${selected?.code??"—"}`} subtitle={`Histórico do shopping selecionado · ${historyPeriod}`} icon={Activity} className="compact-fill-panel">
     {!detail?.history?.length?<EmptyState title="Sem histórico disponível" description="O gráfico será exibido quando houver telemetria histórica."/>:<>
       <div className="energy-main-chart min-h-0 flex-1"><ResponsiveContainer width="100%" height="100%"><LineChart data={detail.history} margin={{top:8,right:8,bottom:2,left:-8}}><CartesianGrid stroke="var(--border)" strokeDasharray="3 3" vertical={false}/><XAxis dataKey="timestamp" tickFormatter={v=>timeLabel(String(v))} minTickGap={28} tick={{fontSize:9,fill:"var(--muted-foreground)"}}/><YAxis tick={{fontSize:9,fill:"var(--muted-foreground)"}} width={52}/><Tooltip contentStyle={chartTooltipStyle} labelFormatter={v=>new Date(String(v)).toLocaleString("pt-BR")} formatter={(value,name)=>[formatNumber(Number(value),{maximumFractionDigits:1}),String(name)]}/><Line type="monotone" dataKey="energyKwh" name="Energia (kWh)" stroke="var(--accent-blue)" dot={false} strokeWidth={2} connectNulls/><Line type="monotone" dataKey="savedKwh" name="Energia evitada (kWh)" stroke="var(--accent-green)" dot={false} strokeWidth={2} connectNulls/></LineChart></ResponsiveContainer></div>
