...
export function ShoppingCard({ shopping }: { shopping: Shopping }) {
  ...
  return (
    <Link
      ...
      title={shopping.name}
      className="group relative flex min-h-[110px] flex-col overflow-hidden rounded-xl border border-border/55 bg-[linear-gradient(145deg,color-mix(in_oklab,var(--card)_96%,transparent),color-mix(in_oklab,var(--background)_82%,transparent))] p-2.5 transition-all duration-200 hover:-translate-y-0.5 hover:border-[color-mix(in_oklab,var(--accent-cyan)_48%,transparent)] hover:shadow-[0_12px_32px_-22px_color-mix(in_oklab,var(--accent-cyan)_55%,transparent)]"
    >
      <span
      ...
      <div className="flex items-center gap-3 pr-5">
        <div className="grid h-8 w-8 shrink-0 place-items-center rounded-full border border-[color-mix(in_oklab,var(--accent-blue)_28%,transparent)] bg-[radial-gradient(circle_at_35%_30%,color-mix(in_oklab,var(--accent-blue)_28%,transparent),color-mix(in_oklab,var(--card)_94%,transparent)_72%)] shadow-[inset_0_1px_0_oklch(1_0_0/8%)]">
          <Building2 className="h-4 w-4 text-[var(--accent-blue)]" strokeWidth={1.8} />
        </div>
        <div className="min-w-0">
          <div className="text-[14px] font-semibold tracking-wide text-foreground">
            {shopping.code}
          </div>
      ...
      <div className="mt-1.5 grid grid-cols-3 divide-x divide-border/55 text-center">
        <Metric
          value={
...
function Metric({ value, unit, label }: { value: string; unit: string; label: string }) {
  return (
    <div className="px-1.5 py-1">
      <div className="metric-value text-[14px] leading-none text-foreground">{value}</div>
