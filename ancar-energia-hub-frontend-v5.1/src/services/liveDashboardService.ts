...
export function mapLiveShoppingToLegacy(item: LiveShoppingSummary): Shopping {
  ...
  return {
    ...
    targetChillerKwTr: asNumber(item.settings?.targetChillerKwTr),
    energyTariffBrlMwh: asNumber(item.settings?.energyTariffBrlMwh),
    energyCostTodayBrl: asNumber(item.today?.energyCostBrl),
    costAboveTargetTodayBrl: asNumber(item.today?.costAboveTargetBrl),
...
export function buildCurrentAlerts(items: LiveShoppingSummary[]): Alert[] {
  ...
  for (const item of items) {
    ...
    const technicalStale = Math.max(10, (item.collectionIntervalMinutes ?? 3) * 3);
    if (ageMinutes > technicalStale) {
      alerts.push(makeAlert(item, "stale", "critico", "Comunicação degradada", `Última coleta há ${Math.round(ageMinutes)} minutos.`, "Verifique comunicação, n8n e WebCTRL.", date));
    }
    const healthStatus = String(latest.health?.status ?? latest.qualityStatus ?? "");
    if (healthStatus === "partial" || healthStatus === "error") {
      alerts.push(makeAlert(item, "qualidade", healthStatus === "error" ? "critico" : "atencao", "Qualidade de aquisição reduzida", `${latest.health?.pointsOk ?? 0} de ${latest.health?.pointsTotal ?? item.registry.pointsTotal} pontos válidos no último ciclo.`, "Verifique os pontos com falha e as tentativas de retry.", date));
    }
    ...
    if (kwTr !== null && target !== null && target > 0 && kwTr > target) {
      ...
      const economic = asNumber(latest.kpis?.potencial_economia_brl_h);
      alerts.push(makeAlert(item, "meta-cag", "atencao", "CAG acima da meta de eficiência", `${fmt(deviation)}% acima da meta${economic!==null?`; oportunidade estimada de R$ ${fmt(economic,2)}/h`:""}.`, "Avalie carga, sequência de chillers e periféricos.", date));
    }
    const targetChiller = asNumber(settings.targetChillerKwTr);
    if (targetChiller !== null && targetChiller > 0) {
      for (const [equipmentKey, eq] of Object.entries(latest.kpis?.equipamentos ?? {})) {
        const eqKwTr = asNumber(eq?.kw_tr);
        if (eqKwTr === null || eqKwTr <= targetChiller) continue;
        const deviation = ((eqKwTr-targetChiller)/targetChiller)*100;
        const opp = asNumber(eq?.opportunity_brl_h);
        alerts.push({ id:`${item.code}-${equipmentKey}-meta`, shoppingId:item.id, shoppingCode:item.code, shoppingName:item.name, equipment:equipmentKey, severity:"atencao", status:"novo", date, title:`${equipmentKey.replaceAll("_"," ").toUpperCase()} acima da meta`, description:`Eficiência ${fmt(eqKwTr,2)} kW/TR (${fmt(deviation)}% acima da meta)${opp!==null?`; oportunidade estimada R$ ${fmt(opp,2)}/h`:""}.`, recommendation:"Avalie distribuição de carga, setpoints e condição do equipamento." });
      }
    }
  }
...
export const liveDashboardService = {
  async getPortfolio(): Promise<PortfolioApiResponse> {
    ...
    result.shoppings = result.shoppings.map((s) => ({ ...s, settings: normalizeSettings(s.settings), today: s.today ?? { energyKwh: null, thermalTrh: null, savedKwh: null, avoidedKgCo2: null, avgKwTr: null, energyCostBrl:null, targetEnergyKwh:null, excessEnergyKwh:null, costAboveTargetBrl:null, costPerTrhBrl:null } }));
    return result;
  },
  ...
  async getShopping(code: string, period: HistoryPeriod = "24h"): Promise<ShoppingApiResponse> {
    ...
    result.summary = result.summary ?? { energyKwh: null, thermalTrh: null, savedKwh: null, avoidedKgCo2: null, avgKwTr: null, avgKw: null, maxKw: null, avgTr: null, maxTr: null, avgAuxKw: null, avgDataQualityPct: null, energyCostBrl:null, targetEnergyKwh:null, excessEnergyKwh:null, costAboveTargetBrl:null, costPerTrhBrl:null, targetTimePct:null };
  ...
  async saveSettings(code: string, settings: ShoppingSettings): Promise<SettingsApiResponse> {
    const result = await requestJson<SettingsApiResponse>(`${API_BASE_URL}/ancar-settings-v1`, {
      method: "PUT",
      body: JSON.stringify({ shoppingId: code, targetKwTr: settings.targetKwTr, targetChillerKwTr: settings.targetChillerKwTr, energyTariffBrlMwh: settings.energyTariffBrlMwh, updatedBy: "frontend" }),
    });
...
export function normalizeSettings(input?: Partial<ShoppingSettings> | null): ShoppingSettings {
  return {
    ...
    targetChillerKwTr: asNumber(input?.targetChillerKwTr),
    energyTariffBrlMwh: asNumber(input?.energyTariffBrlMwh),
